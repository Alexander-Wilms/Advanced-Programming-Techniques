		...
		matrixInit_t init)
{
	...
	case ZERO :
		...
		{
			m_pData[i] = CCoordinate(0,0);
		}
		break;
	case ONE :
		for (unsigned int i = 0; i < m_noColumns * m_noRows; i++)
		{
			m_pData[i] = CCoordinate(1,1);
		}
		break;
	case INDEX :
		for (unsigned int i = 0; i < m_noColumns * m_noRows; i++)
		{
			m_pData[i] = CCoordinate(i,i);
		}
		break;
	case UNIT:
		for (unsigned int i = 0; i < m_noColumns * m_noRows; i++)
		...
		if (m_noRows == m_noColumns)
		{
			...
			{
				m_pData[getIndex(i,i)] = CCoordinate(1,1);
			}
		}
		else
		{
...
void CMatrix::print() const
{
	for (int r = 0; r < m_noRows; r++)
	{
		...
		{
			//More eleant: overloaded operators ==> later
			//cout << m_pData[getIndex(r,c)] << " ";
			m_pData[getIndex(r,c)].print();
		}
		cout << endl;
	}
